...
export function useSeasons() {
  ...
  const fetchSeasons = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        const fetchedSeasons = result.data.seasons || [];
        setSeasons(fetchedSeasons);

        // Find the current season from the is_current field
        const currentSeason = fetchedSeasons.find((s: Season) => s.is_current === 1);
        if (currentSeason) {
  ...
  const setCurrentSeason = async (seasonId: number) => {
    ...
    try {
      const result = await window.api.touchline.sendCommand("seasons.set_current", {
        season_id: seasonId
      });
